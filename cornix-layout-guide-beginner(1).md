...
| `'` `=` | (where the Right and Down arrow keys normally are) |
...
**New: arrow keys on H J K L.** While holding Red dot, H/J/K/L now work like they do in Vim:
...
This means you can move a cursor around without ever leaving your home row — useful for scrolling terminal output, reading long pages, or just navigating without reaching for the arrow cluster.

Curly braces `{` `}` used to live on L/K but were removed to make room for the arrows. You can still type them: hold real Shift **and** Red dot, then press A or S (which normally give you `[` and `]`) — Shift turns those into `{` and `}`.
