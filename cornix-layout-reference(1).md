...
| M | `&` |
| N | `^` |
| Right (arrow key) | `'` (quote) |
| Down (arrow key) | `=` |
| Left (arrow key), Green-dot, Raise, Space (right thumb) | *(inactive)* |
...
**v3 update:** L/K/J/H now send vim-style arrow keys (Right/Up/Down/Left) instead of `{ } ' =`. Quote and equal were relocated to the Right and Down arrow-key positions on this same layer. Curly braces `{ }` were dropped from this layer entirely — get them by holding real Shift + Red-dot + A/S (which send `[`/`]`; Shift turns them into `{`/`}`).
